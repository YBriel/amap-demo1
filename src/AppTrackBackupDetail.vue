...
export default {
  ...
  methods: {
    refreshLine :  async function ()  {
      let strings = this.input.split(";");
      this.path[0].path = [];
      let origin = strings[0];
      console.log("起点"+origin)
      let dest = strings[strings.length - 1];
      console.log("终点"+dest+strings.length );
      strings.shift();
      strings.pop();
      let wayPoints='';
      for (let i = 0; i < strings.length; i++) {
        wayPoints=wayPoints+strings[i]+";";
      }
      console.log(wayPoints)
    let routesRes =await this.getRoutes(origin,dest,wayPoints);
    let  routes=routesRes.split(";")
       for (let i = 1; i < routes.length - 1; i++) {
         let arr = [];
         let string = routes[i].split(",");
